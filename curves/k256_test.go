...
func TestP256PointCompression(t *testing.T) {
	kryptologyk256 := curves.CurveK256()
...
func TestScalarK256Random(t *testing.T) {
	...
	require.Equal(t, s.BigInt(), expected)
...
func TestScalarK256Sqrt(t *testing.T) {
	...
	require.NoError(t, err)
	fmt.Printf("SA: 0x%x\tXPTD: 0x%x\n", sa.Bytes(), expected.Bytes())
	require.Equal(t, sa.Cmp(expected), 0)
}
...
func TestPointK256Random(t *testing.T) {
	...
	require.Equal(t, s.value.X.BigInt(new(big.Int)), expectedX)
	require.Equal(t, s.value.Y.BigInt(new(big.Int)), expectedY)
	// Try 10 random values
	for i := 0; i < 10; i++ {
...
func TestPointK256Generator(t *testing.T) {
	...
	s, ok := sc.(*PointK256)
	require.True(t, ok)
	require.Equal(t, s.value.X.BigInt(new(big.Int)).Cmp(btcec.S256().Gx), 0)
	require.Equal(t, s.value.Y.BigInt(new(big.Int)).Cmp(btcec.S256().Gy), 0)
}
