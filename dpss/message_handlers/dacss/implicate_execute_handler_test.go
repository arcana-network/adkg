...
Testcase: the share for which the implicate was sent, is empty.
This is stored in the node's state (acssState.acssData)

Expectation: early return; no msg is sent
*/
func TestShareLenZero(t *testing.T) {
	node1, node2, executeMsg, acssRoundDetails := happyPathSetup()

	// Set share to empty
	node1.State().AcssStore.UpdateAccsState(acssRoundDetails.ToACSSRoundID(), func(state *common.AccsState) {
		state.AcssData.ShareMap[common.PointToHex(node2.LongtermKey.PublicKey)] = []byte{}
	})
...
func invalidateShareForNode(node2 *testutils.PssTestNode, node1 *testutils.PssTestNode, acssRoundDetails common.ACSSRoundDetails) {
	...
	node1.State().AcssStore.UpdateAccsState(acssRoundDetails.ToACSSRoundID(), func(state *common.AccsState) {
		state.AcssData.ShareMap[node2PubkeyHex] = bytes
	})
}
